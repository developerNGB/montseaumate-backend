 ...
 * Uses the Gmail REST API over HTTPS (port 443) as the primary strategy to bypass SMTP port blocking.
 */
export async function sendAdminNotification({ subject, text, html, replyTo }) {
    ...
    try {
        ...
        if (supportHost && supportUser && supportPass && !isPlaceholderSmtpPass(supportPass)) {
            console.log(`[AdminMailService] Attempting CDMON SMTP send via ${supportUser} to ${adminEmail}`);
    ...
    // --- STRATEGY 2: Try env Gmail SMTP (port 465 -> 587 fallback) ---
    const gmailUser = (process.env.EMAIL_USER || 'equipoexpertoia@gmail.com').trim();
    const gmailPass = (process.env.EMAIL_PASS || '').replace(/\s+/g, '');
    if (gmailUser && gmailPass && !isPlaceholderSmtpPass(gmailPass)) {
        // Try Port 465 first
        try {
            console.log(`[AdminMailService] Attempting env Gmail SMTP:465 send to ${adminEmail}`);
        ...
        // Try Port 587 next
        try {
            console.log(`[AdminMailService] Attempting env Gmail SMTP:587 send to ${adminEmail}`);
    ...
    // --- STRATEGY 3: Try env Gmail REST API over HTTPS (using GOOGLE_REFRESH_TOKEN) ---
    try {
    ...
    // --- STRATEGY 4: Try Database-configured Google OAuth integration for the admin user ---
    try {
        ...
        if (targetUserId) {
            ...
            if (integration) {
                ...
                if (fromEmail) {
                    ...
                    if (accessToken) {
                        console.log(`[AdminMailService] Attempting Gmail API send (From: ${fromEmail} To: ${adminEmail})`);
                        const mailOptions = {
                            to: adminEmail,
                            replyTo: replyTo || undefined,
                            subject,
    ...
    // --- STRATEGY 5: Try Database-configured SMTP Settings for the admin user ---
    try {
        const adminRes = await pool.query(`SELECT id FROM users WHERE lower(email) = lower($1) LIMIT 1`, [adminEmail]);
        const targetUserId = adminRes.rows[0]?.id || (await pool.query(`SELECT id FROM users ORDER BY created_at ASC LIMIT 1`)).rows[0]?.id;
        ...
        if (targetUserId) {
            ...
            if (config) {
                console.log(`[AdminMailService] Attempting DB SMTP send via ${config.from_email} to ${adminEmail}`);
                const transporter = nodemailer.createTransport({
                    host: config.host,
                    port: config.port,
